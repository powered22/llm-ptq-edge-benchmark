#!/usr/bin/env bash
# Run all quantization methods on Qwen2.5-0.5B-Instruct.
...
run_step "[3/6] GPTQ W8A16" \
  python quantization/quantize_gptq_v2.py \
    --model "$MODEL" \
    --output "$OUT_DIR/qwen2.5_0.5b_instruct_gptq_w8a16" \
    --num-samples "$NUM_SAMPLES" \
    --scheme W8A16
...
run_step "[4/6] RTN W4A16" \
  python quantization/quantize_rtn.py \
    --model "$MODEL" \
    --output "$OUT_DIR/qwen2.5_0.5b_instruct_rtn_w4a16" \
    --scheme W4A16
...
run_step "[6/6] SmoothQuant W8A8" \
  python quantization/quantize_smoothquant.py \
    --model "$MODEL" \
    --output "$OUT_DIR/qwen2.5_0.5b_instruct_smooth_w8a8"

echo ""
echo "=========================================="
echo "  All 6 quantization runs complete."
echo "  Output: $OUT_DIR/qwen2.5_0.5b_instruct_*"
echo "=========================================="
